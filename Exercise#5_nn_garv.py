...
# -*- coding: utf-8 -*-
"""
Created on Sat Nov 12 01:53:47 2022
...
z = np.random.uniform(min_val, max_val, num_points)
z = z.reshape(num_points, 1)
input_garv = np.append(x, y, axis=1)
input_garv=np.append(input_garv, z, axis=1)
feature1 = input_garv[:,0]
feature2 = input_garv[:,1]
feature3 = input_garv[:,2]
"""generating output by adding two features simaltaniously"""
output_garv = feature1+feature2+feature3
...
dim3_min, dim3_max = input_garv[:,2].min(), input_garv[:,2].max()
...
error_progress = nn.train(input_garv, output_garv, show=15, goal=0.00001)
...
"""using numpy seed generating constant data for model testing"""
np.random.seed(1)
"""Generating uniform random values in x and y using min and max values and total of 100(num_points) and then 
reshaping it into rows and columbs so that we can put it into one array"""
x = np.random.uniform(min_val, max_val, num_points)
x = x.reshape(num_points, 1)
y = np.random.uniform(min_val, max_val, num_points)
